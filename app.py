...
def index():
    ...
    transactions_data = db.execute("SELECT symbol, SUM(shares) AS shares, price FROM transactions WHERE user_id = ? GROUP BY symbol", user_id)
    cash_data = db.execute("SELECT cash FROM users WHERE id = ?", user_id)
    cash = cash_data[0]["cash"]

    return render_template("index.html", database = transactions_data, cash = cash)
...
def buy():
    ...
    user_cash = user_cash_data[0]["cash"]
...
def history():
    ...
    user_id = session["user_id"]
    transactions_data = db.execute("SELECT * FROM transactions WHERE user_id = :id", id=user_id)
...
def register():
    ...
    if request.method == "GET":
        ...
        if password != confirmation:
            return apology("Password & confirmation do not match")
...
@login_required
def add_cash():
    """Allowing user to add some cash"""
    if request.method == "GET":
        ...
        if not new_cash:
            return apology("Give amount of money to add")

        if new_cash <= 0:
            return apology("Positive number of cash required")
